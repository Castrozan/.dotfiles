...
def resolve_modern_python_absolute_path() -> str:
    for candidate in [shutil.which("python3"), *MODERN_PYTHON_CANDIDATE_PATHS]:
        ...
        if minor.isdigit() and int(minor) >= EARLIEST_PYTHON_MINOR_THE_VALIDATORS_PARSE:
            return candidate
    pytest.skip("no python3 >= 3.10 available to run the baseline validators")


def command_poor_bin_directory(tmp_path: pathlib.Path) -> pathlib.Path:
    interpreter = resolve_modern_python_absolute_path()
...
def tracked_baseline(file_name: str, **overrides) -> dict:
    baseline = {
