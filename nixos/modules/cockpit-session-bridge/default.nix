...
in
{
  options.custom.cockpitSessionBridge = {
    ...
    persistentSession = {
      enable = lib.mkOption {
        ...
        description = "Keep an always-on tmux session running a host-configured TUI (e.g. chise's openclaw jarvis bot) so the cockpit Internal terminal attaches to one shared live TUI, the way the clawde agents stay resident, instead of spawning a throwaway shell per connection.";
      };
  ...
  config = lib.mkIf cockpitSessionBridgeConfig.enable {
    systemd.services.jarvis-session-tmux = lib.mkIf persistentSessionConfig.enable {
      description = "Jarvis always-on cockpit TUI tmux session";
