...
setup() {
	export SCRIPT_UNDER_TEST="$DOTFILES_ROOT_DIRECTORY/machine-configuration/terminal/multiplexer/tmux/program-configuration/binds.conf"
}

@test "every toggle binding backgrounds its dispatch with run-shell -b" {
	for key in i b e v; do
...
@test "no toggle binding falls back to a blocking foreground run-shell" {
	assert_script_source_does_not_match "^bind [ibev] run-shell \""
}
