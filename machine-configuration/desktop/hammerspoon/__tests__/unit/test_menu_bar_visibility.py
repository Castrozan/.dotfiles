...
def configure_visibility(reveal_module, monkeypatch, results=(0, 0)):
    ...
    monkeypatch.setattr(reveal_module.time, "sleep", wait)
    return library.SLSSetMenuBarVisibilityOverrideOnDisplay, wait


def test_reveal_is_display_scoped_and_releases_after_one_second(
    reveal_module, monkeypatch
):
    visibility, wait = configure_visibility(reveal_module, monkeypatch)
...
def test_termination_interrupts_the_wait_and_releases_visibility(
    ...
    wait.side_effect = terminate_during_wait
    reveal_module.reveal_menu_bar(7)

    assert visibility.call_args_list[-1].args == (42, 7, False)


def test_cancellation_during_reveal_still_releases_visibility(
    reveal_module, monkeypatch
):
    visibility, wait = configure_visibility(reveal_module, monkeypatch)
    visibility.side_effect = (reveal_module.RevealCancelled(), 0)
...
def test_wait_failure_still_releases_visibility(reveal_module, monkeypatch):
    visibility, wait = configure_visibility(reveal_module, monkeypatch)
    wait.side_effect = RuntimeError("interrupted")
...
def test_rejected_reveal_releases_any_partial_override_without_waiting(
    reveal_module, monkeypatch
):
    visibility, wait = configure_visibility(reveal_module, monkeypatch, (1001, 0))
