...
in
{
  home.activation.setupLidSwitchIgnore = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if [ -f /etc/NIXOS ]; then
      $VERBOSE_ECHO "Skipping lid switch setup on NixOS (use machine-configuration/operating-system/power-management/lid-switch-nixos.nix)"
      ...
      sudo "${dotfilesDir}/machine-configuration/operating-system/power-management/scripts/setup-lid-switch-ignore"
    fi
  '';
}
