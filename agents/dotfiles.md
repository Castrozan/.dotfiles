...
Browser devtools MCP servers are gated to the browsers a machine actually runs. Only chise runs Vivaldi, so the `vivaldi-devtools` MCP is injected only when `hostname == "chise"`, in both consumers: the claude MCP server set in `home/base/claude/mcps` and the codex config generator in `home/base/codex`. Do not reintroduce it unconditionally; a browser MCP whose browser is absent still spawns a stdio server that can never connect and wastes memory across every session (kira alone held ~7 dead vivaldi servers). The codex `~/.codex/config.toml` is regenerated by merging into the existing file, so the generator prunes managed browser-MCP names it no longer emits; without that prune a `vivaldi-devtools` entry written by a prior rebuild silently persists on kira and rin after the gate stops emitting it.
</browser-mcp-host-gating>
