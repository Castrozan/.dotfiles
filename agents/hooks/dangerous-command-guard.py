...
CATASTROPHIC_PATTERNS_THAT_BLOCK_EXECUTION = [
    ...
    (
        r"rm\s+-rf\s+\$HOME\s*$",
        "BLOCKED: rm -rf $HOME would delete entire home directory",
    ),
    ...
    (
        r"mkfs\.[a-z]+\s+/dev/sd[a-z][0-9]?\s*$",
        "BLOCKED: mkfs would format the partition",
    ),
    (r":()\s*{\s*:\s*\|\s*:\s*&\s*}", "BLOCKED: Fork bomb detected"),
    (
        r"chmod\s+-R\s+777\s+/\s*$",
        "BLOCKED: chmod 777 / would break system security",
    ),
    (
        r"chown\s+-R\s+.*\s+/\s*$",
        "BLOCKED: chown -R / could break system permissions",
    ),
]

DANGEROUS_PATTERNS_WITH_STRONG_WARNING = [
    ...
    (
        r"git\s+push\s+.*--force.*origin.*(main|master)",
        "Dangerous: Force push to main/master",
    ),
    (
        r"git\s+reset\s+--hard\s+HEAD~\d+",
        "Dangerous: Hard reset discarding commits",
    ),
...
def main():
    ...
    for pattern, message in CATASTROPHIC_PATTERNS_THAT_BLOCK_EXECUTION:
    ...
    for pattern, message in DANGEROUS_PATTERNS_WITH_STRONG_WARNING:
        if re.search(pattern, command, re.IGNORECASE):
            output = {
                "continue": True,
                "systemMessage": (
    ...
    collected_warnings = []
    for pattern, message in MILD_WARNING_PATTERNS:
        if re.search(pattern, command, re.IGNORECASE):
