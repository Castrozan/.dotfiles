...
class TestHashRecallPathSet:
    def test_hash_is_order_independent(self):
        assert memory_recall_debounce.hash_recall_path_set(
            ["/a/one.md", "/a/two.md"]
        ) == memory_recall_debounce.hash_recall_path_set(["/a/two.md", "/a/one.md"])

    def test_distinct_sets_hash_differently(self):
        assert memory_recall_debounce.hash_recall_path_set(
            ["/a/one.md"]
        ) != memory_recall_debounce.hash_recall_path_set(["/a/two.md"])


class TestHasRecallSessionBudgetBeenExhausted:
    def test_empty_state_is_not_exhausted(self):
        assert not memory_recall_debounce.has_recall_session_budget_been_exhausted({})

    def test_event_count_at_budget_is_exhausted(self):
        state = {
            "recall_event_count": memory_recall_debounce.SESSION_RECALL_EVENT_BUDGET
        }
        assert memory_recall_debounce.has_recall_session_budget_been_exhausted(state)

    def test_character_total_at_budget_is_exhausted(self):
        state = {
            "recall_character_total": memory_recall_debounce.SESSION_RECALL_CHARACTER_BUDGET
        }
        assert memory_recall_debounce.has_recall_session_budget_been_exhausted(state)
...
class TestWasRecallPathSetAlreadyInjected:
    def test_unseen_path_set_is_not_already_injected(self):
        assert not memory_recall_debounce.was_recall_path_set_already_injected(
            {}, ["/a/one.md"]
        )
    ...
    def test_recorded_path_set_is_already_injected(self):
        state = {
            "injected_recall_path_set_hashes": [
                memory_recall_debounce.hash_recall_path_set(["/a/one.md"])
            ]
        }
        assert memory_recall_debounce.was_recall_path_set_already_injected(
            state, ["/a/one.md"]
        )
    ...
    def test_path_set_match_ignores_order(self):
        state = {
            "injected_recall_path_set_hashes": [
                memory_recall_debounce.hash_recall_path_set(["/a/one.md", "/a/two.md"])
            ]
        }
        assert memory_recall_debounce.was_recall_path_set_already_injected(
...
class TestRecordRecallInjection:
    def test_injection_increments_counters_and_records_hash(self, tmp_path):
        ...
        memory_recall_debounce.record_recall_injection(state_path, ["/a/one.md"], 42)
    ...
    def test_injection_preserves_existing_debounce_fields(self, tmp_path):
        state_path = tmp_path / "memory-recall-session.json"
        memory_recall_debounce.persist_debounce_state(state_path, ["api", "server"])
        memory_recall_debounce.record_recall_injection(state_path, ["/a/one.md"], 10)
        state = memory_recall_debounce.load_debounce_state(state_path)
...
class TestPersistDebounceStatePreservesBudgetFields:
    def test_persist_after_injection_keeps_budget_and_hash_fields(self, tmp_path):
        state_path = tmp_path / "memory-recall-session.json"
        memory_recall_debounce.record_recall_injection(state_path, ["/a/one.md"], 1234)
        memory_recall_debounce.persist_debounce_state(state_path, ["fresh", "keywords"])
        state = memory_recall_debounce.load_debounce_state(state_path)
        assert state["recall_event_count"] == 1
...
class TestRecordRecallSuppression:
    def test_suppression_increments_reason_count(self, tmp_path):
        state_path = tmp_path / "memory-recall-session.json"
        memory_recall_debounce.record_recall_suppression(
            state_path, memory_recall_debounce.SUPPRESSION_REASON_BUDGET
        )
        state = memory_recall_debounce.load_debounce_state(state_path)
        assert state["suppressed_event_count_by_reason"] == {
            memory_recall_debounce.SUPPRESSION_REASON_BUDGET: 1
        }

    def test_distinct_reasons_count_independently(self, tmp_path):
        state_path = tmp_path / "memory-recall-session.json"
        memory_recall_debounce.record_recall_suppression(
            state_path, memory_recall_debounce.SUPPRESSION_REASON_DEBOUNCE
        )
        memory_recall_debounce.record_recall_suppression(
            state_path, memory_recall_debounce.SUPPRESSION_REASON_DEBOUNCE
        )
        memory_recall_debounce.record_recall_suppression(
            state_path, memory_recall_debounce.SUPPRESSION_REASON_DEDUP, 300
        )
        state = memory_recall_debounce.load_debounce_state(state_path)
        assert state["suppressed_event_count_by_reason"] == {
            memory_recall_debounce.SUPPRESSION_REASON_DEBOUNCE: 2,
            memory_recall_debounce.SUPPRESSION_REASON_DEDUP: 1,
        }
    ...
    def test_only_dedup_character_count_accumulates(self, tmp_path):
        ...
        memory_recall_debounce.record_recall_suppression(
            state_path, memory_recall_debounce.SUPPRESSION_REASON_DEDUP, 512
        )
        memory_recall_debounce.record_recall_suppression(
            state_path, memory_recall_debounce.SUPPRESSION_REASON_DEDUP, 488
        )
        state = memory_recall_debounce.load_debounce_state(state_path)
    ...
    def test_suppression_preserves_existing_budget_and_debounce_fields(self, tmp_path):
        ...
        memory_recall_debounce.persist_debounce_state(state_path, ["api", "server"])
        memory_recall_debounce.record_recall_suppression(
            state_path, memory_recall_debounce.SUPPRESSION_REASON_DEDUP, 7
        )
        state = memory_recall_debounce.load_debounce_state(state_path)
        assert state["recall_event_count"] == 1
        assert state["recall_character_total"] == 42
