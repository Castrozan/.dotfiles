import memory_recall_keywords


class TestExtractKeywordsFromText:
    def test_filters_english_stop_words(self):
        keywords = memory_recall_keywords.extract_keywords_from_text(
    ...
    def test_filters_tokens_below_min_length(self):
        keywords = memory_recall_keywords.extract_keywords_from_text("ab abc abcd")
    ...
    def test_caps_at_max_keywords(self):
        ...
        keywords = memory_recall_keywords.extract_keywords_from_text(text)
        assert len(keywords) <= memory_recall_keywords.MAX_KEYWORDS

    def test_lowercases_all_keywords(self):
        keywords = memory_recall_keywords.extract_keywords_from_text(
            "APIClient ServerError"
        )
        assert "apiclient" in keywords
        assert "servererror" in keywords

    def test_returns_empty_on_empty_text(self):
        assert memory_recall_keywords.extract_keywords_from_text("") == []
...
class TestCollectStringsFromToolInput:
    def test_handles_plain_string(self):
        assert (
            memory_recall_keywords.collect_strings_from_tool_input("hello world")
            == "hello world"
        )

    def test_handles_dict_values(self):
        result = memory_recall_keywords.collect_strings_from_tool_input(
    ...
    def test_handles_none_and_numbers(self):
        assert memory_recall_keywords.collect_strings_from_tool_input(None) == ""
        assert memory_recall_keywords.collect_strings_from_tool_input(42) == ""
