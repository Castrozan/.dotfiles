...
Name matching is case-insensitive per-word — "vitor bonfante" matches "Vitor Vassoler Bonfante". Images are sent via clipboard paste (wl-copy + Ctrl+V). Claude Code images live at `~/.claude/image-cache/{session-id}/{N}.png`, OpenClaw at `~/.openclaw/media/inbound/`.
