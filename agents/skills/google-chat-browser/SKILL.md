...
<traps>
Name matching is case-insensitive per-word. Contact resolution tries sidebar first, then "show all" expansion, then Google Chat's search bar as fallback. Images are sent via clipboard paste (wl-copy + Ctrl+V). Claude Code images live at ~/.claude/image-cache/{session-id}/, OpenClaw at ~/.openclaw/media/inbound/.
</traps>
