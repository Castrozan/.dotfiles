...
Tweet media enrichment: when tweets contain `pic.twitter.com` or `t.co` links and the user asks about attached images or video, use the top-level `twitter` skill for twikit-cli usage and the fxtwitter API for reliable media URLs without auth. Do not reach for the browser skill to view tweet content.
</inbox_processing>
