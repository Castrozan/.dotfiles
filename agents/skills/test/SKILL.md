...
2. Compare your implementation against every point in their request. Did you miss anything? Did you add anything they
   didn't ask for?
3. Push the complete change set, not just the last file you touched, and wait out CI with `dotfiles-ci`.
...
`dotfiles-ci` waits on that run and reports every failure, so a local full-suite pass is not what proves a change. Do
