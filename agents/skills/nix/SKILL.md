...
Umbrella skill for Nix language expertise, rebuild workflow, devenv shells, this repository's conventions, and container
management. Each capability lives in its own file so only the relevant one loads into context.
...
For project-level dev shells with devenv: entering, updating traps, cleaning stale state, why direnv is avoided; read
`devenv.md`.
