---
name: comms
description: Send Google Chat messages, DMs, webhooks. Post and scrape Twitter/X tweets, profiles, trends. Control VTuber avatar lip sync and expressions.
---

Unified communication and social media skill covering messaging, social platforms, and avatar control.

For Google Chat browser automation and webhooks, read `skills/google-chat/SKILL.md`.
For Twitter/X scraping, posting, and account operations, read `skills/twitter/SKILL.md`.
For VTuber avatar control, speech, and expressions, read `skills/avatar/SKILL.md`.
