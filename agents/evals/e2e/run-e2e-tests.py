...
def place_claude_md_in_workspace(
    ...
    (workspace_directory / "AGENTS.md").write_text(agents_md_content)
...
def setup_e2e_scenario_workspace(
    ...
    place_claude_md_in_workspace(workspace_directory)
...
def run_e2e_scenario(
    ...
    workspace = Path(tempfile.mkdtemp(prefix=f"e2e-{sanitized}-"))
    timeout = scenario.get("timeout", 300)

    try:
        setup_e2e_scenario_workspace(scenario, workspace)
