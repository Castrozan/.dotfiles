...
let
  ...
  installOpenclawViaNpm = pkgs.writeShellScript "openclaw-install" ''
    ...
    DISCORD_VOICE_PACKAGE_JSON="${npmPrefix}/lib/node_modules/@discordjs/voice/package.json"
    ...
    DISCORD_VOICE_READY=false
    if [ -f "$DISCORD_VOICE_PACKAGE_JSON" ] && grep -q '"version": "${discordVoicePackageVersion}"' "$DISCORD_VOICE_PACKAGE_JSON"; then
      DISCORD_VOICE_READY=true
    fi

    if [ "$OPENCLAW_READY" = true ] && [ "$DAVEY_READY" = true ] && [ "$DISCORD_JS_READY" = true ] && [ "$DISCORD_VOICE_READY" = true ]; then
      exit 0
    fi

    ${nodejs}/bin/npm install -g "openclaw@${version}" "@snazzah/davey@${discordVoiceDavePackageVersion}" "discord.js@${discordJsPackageVersion}" "@discordjs/voice@${discordVoicePackageVersion}" \
      --prefix "${npmPrefix}" \
      --ignore-scripts \
      --registry "https://registry.npmjs.org/"

    ${nodejs}/bin/node "${config.home.homeDirectory}/.dotfiles/home/modules/openclaw/openclaw-voice-playback-patch.js"
  '';
