...
class TestCalculateContrastRatio:
    def test_black_on_white_is_maximum(self):
        ratio = theme_generate_from_wallpaper.calculate_contrast_ratio(
            (0, 0, 0), (255, 255, 255)
        )
    ...
    def test_is_symmetric(self):
        forward = theme_generate_from_wallpaper.calculate_contrast_ratio(
            (21, 55, 168), (5, 7, 14)
        )
        backward = theme_generate_from_wallpaper.calculate_contrast_ratio(
            (5, 7, 14), (21, 55, 168)
        )
...
class TestLightenColorUntilMinimumContrast:
    def test_preserves_hue(self):
        background = (5, 7, 14)
        original = (21, 55, 168)
        lifted = theme_generate_from_wallpaper.lighten_color_until_minimum_contrast(
            original, background, 4.5
        )
        original_hue = theme_generate_from_wallpaper.color_to_hls(original)[0]
        lifted_hue = theme_generate_from_wallpaper.color_to_hls(lifted)[0]
...
class TestAccentColorsMeetMinimumContrastAgainstBackground:
    def test_all_accent_slots_meet_minimum_contrast(self):
        ...
        floor = theme_generate_from_wallpaper.MINIMUM_ACCENT_CONTRAST_RATIO
    ...
    def test_brights_remain_at_least_as_bright_as_normals(self):
        ...
        for normal_slot in [1, 2, 3, 4, 5, 6]:
            normal_luminance = theme_generate_from_wallpaper.calculate_yiq_luminance(
                *palette[normal_slot]
            )
            bright_luminance = theme_generate_from_wallpaper.calculate_yiq_luminance(
