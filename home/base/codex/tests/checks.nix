...
in
{
  ...
  codex-config-agent-instructions-current =
    mkEvalCheck "codex-config-agent-instructions-current"
      (
        !(lib.hasInfix "codex config generator" dotfilesAgentInstructions)
        && !(lib.hasInfix "regenerated by merging into the existing file" dotfilesAgentInstructions)
        && lib.hasInfix "home/base/claude/mcps/default.nix" dotfilesAgentInstructions
        && lib.hasInfix "home/base/codex/config.nix" dotfilesAgentInstructions
        && lib.hasInfix "preserving live entries in projects, marketplaces, and plugins" dotfilesAgentInstructions
        && lib.hasInfix "sourced entries win on key collisions" dotfilesAgentInstructions
      )
