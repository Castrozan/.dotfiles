...
| `hooks/`                   | Deploys `agents/hooks/` flat under `~/.claude/hooks/` (`default.nix`), hook event registrations (`event-registrations.nix`), and the recursive hook-tree walker. The walker deploys `.md` too, so only a runtime resource a hook reads belongs beside it; before authoring a hook, read `docs/hook-output-channels.md` for which output key reaches the model and which reaches only the human |
...
| `docs/`                    | Module documentation, not deployed to the live config: context budget and compaction (`context-management.md`), memory architecture (`memory/`), hook output channels (`hook-output-channels.md`)                                                                                                                                                                                              |
