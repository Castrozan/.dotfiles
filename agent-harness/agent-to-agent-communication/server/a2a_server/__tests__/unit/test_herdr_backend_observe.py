...
def _set_target_pane_exists_to(backend, exists):
    ...
    backend._read_pane_information = lambda: (
        {"agent": "claude", "agent_status": "idle"} if exists else {}
    )
...
class TestObserveProducesActivityOnlyForNewMeaningfulLines:
    def test_first_observation_after_start_reports_no_new_lines_when_pane_unchanged(
        ...
        backend = HerdrAttachedAgentBackend("wP:pT", meaningful_line_pattern=pattern)
        _replace_capture_pane_text_with(backend, "⏺ first response\n")
        _set_target_pane_exists_to(backend, True)
        backend.start()
