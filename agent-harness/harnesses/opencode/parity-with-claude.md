...
- The three-block reply shape is content, not chrome, so it ports as an
  ...
  The complete humanize policy remains in `SKILL.md` and loads only when
  substantial human-facing writing requires it. This mirrors how the `codex` wrapper injects
