...
setup() {
	...
	LISTENER_PROCESS_ID=""
	PROXY_STARTUP_TIMEOUT_SECONDS="0"
	...
	PROXY_UNAVAILABLE_MESSAGE="cli-proxy-api is not listening on $PROXY_LISTEN_ADDRESS:$PROXY_LISTEN_PORT.
If you have never authenticated your ChatGPT subscription, run: claudex-login
Otherwise inspect the service: $PROXY_SERVICE_INSPECTION_COMMAND"
...
run_claudex() {
	run env -i \
		...
		CLAUDEX_LAUNCHER_LIFECYCLE_PYTHON="$LIFECYCLE_PYTHON" \
		CLAUDEX_LAUNCHER_LIFECYCLE_SCRIPT="$LIFECYCLE_SCRIPT" \
		CLAUDEX_LAUNCHER_PROXY_REGISTRY_DIRECTORY="$TEMPORARY_ROOT/holders" \
		CLAUDEX_LAUNCHER_PROXY_STARTUP_TIMEOUT_SECONDS="$PROXY_STARTUP_TIMEOUT_SECONDS" \
		CLAUDEX_LAUNCHER_PROXY_SERVICE_CONTROLLER="launchd" \
		CLAUDEX_LAUNCHER_PROXY_SERVICE_LABEL="com.dotfiles.cli-proxy-api.absent-under-test" \
		CLAUDEX_LAUNCHER_PROXY_UNAVAILABLE_MESSAGE="$PROXY_UNAVAILABLE_MESSAGE" \
