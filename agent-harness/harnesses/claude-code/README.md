...
| `private.nix`              | Deploys the shared private subagents and commands from `private-configuration/claude/`. Private skills are not deployed here: they reach a session only through the shared catalog, so this file can never inject one behind the curated list's back                                                                                                                                                   |
