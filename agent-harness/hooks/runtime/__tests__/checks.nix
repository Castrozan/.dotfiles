...
let
  ...
  flatDeployedHookScriptNames = map (entry: entry.flatDeploymentFilename) (
    listHookScriptsRecursively ../../runtime
  );
...
in
{
  hooks-every-event-maps-to-a-flat-deployed-dispatcher =
    mkEvalCheck "hooks-every-event-maps-to-a-flat-deployed-dispatcher"
      ...
      (
        "every event in agent-harness/hooks/runtime/event-to-dispatcher-map.nix must map to a -dispatcher.py that "
        + "exists in the flat deploy of agent-harness/hooks/runtime; a map referencing a missing script ships a "
  ...
  hooks-every-flat-deployed-dispatcher-is-claimed-by-an-event =
    mkEvalCheck "hooks-every-flat-deployed-dispatcher-is-claimed-by-an-event"
      ...
      (
        "every -dispatcher.py flat-deployed from agent-harness/hooks/runtime must be claimed by at least one event "
        + "in agent-harness/hooks/runtime/event-to-dispatcher-map.nix; an unclaimed dispatcher never runs, so "
  ...
  hooks-the-event-map-covers-every-hook-option =
    mkEvalCheck "hooks-the-event-map-covers-every-hook-option"
      ...
      (
        "agent-harness/hooks/runtime/event-to-dispatcher-map.nix is the single definition of which hook option each "
