...
Apply `<source_fidelity>` before selecting or defining a term. When no established name exists, select the clearest
...
<interactive_communication_reference>
Interactive harnesses inject `references/interactive-communication.md` separately because its response contract must
remain available before Humanize is selected. Do not reload it during ordinary Humanize use.
</interactive_communication_reference>
