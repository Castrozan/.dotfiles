...
<troubleshooting>
Lights unavailable but working from the phone app: the Tuya cloud auth token expired. Check the config entries API
...
Tuya auth is expired. New Midea devices need auto-discovery through the config flow in HA. Both scripts follow
identical patterns, so read existing code before modifying.
</adding_devices>
