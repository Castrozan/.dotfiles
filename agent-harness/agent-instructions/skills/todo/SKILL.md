...
<purpose>
Todoist is Lucas's single task tracker across every horizon, today to months out, visible to him in the Todoist
app, browser extension, and phone. Record anything he must do or remember with `todo` rather than in notes or agent
memory, so it surfaces where he actually looks.
</purpose>
...
<semantics>
A task added with no `--due` is intentionally an undated someday/long-horizon backlog item, not a mistake;
give it a date only when it becomes time-bound. `todo digest --json` returns `{overdue, today, someday}`, the shape a
scheduled briefing push consumes.
</semantics>
...
<gotchas>
Priority is Todoist-native where 4 is highest (UI p1), so `--priority 1` for "top priority" silently sets the
lowest instead. `--filter` takes Todoist's own query language (`today`, `overdue`, `no date`, `@label`, `#project`,
`p1`, boolean `|` and `&`). Run `todo --help` or `todo <command> --help` for the authoritative flag surface.
</gotchas>
